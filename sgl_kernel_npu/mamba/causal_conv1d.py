...
def torch_causal_conv1d_update_npu(
    ...
    """
    Channels-last variant used by the decode path.

    hidden_state: (batch, seq_len, dim)
    conv_state:   (batch, state_len, dim)
    weight:       (dim, width)
    out:          (batch, seq_len, dim)
    conv_state_update: (batch, state_len, dim)
    """
    ...
    if conv_state_update is not None:
        ...
        # contiguous() accelerates the fancy-index writeback in the caller
        conv_state_update = hidden_states_new[:, -state_len:, :].contiguous()

    # weight: (dim, width) -> broadcast to (1, width, dim)
    weight_b = weight.transpose(0, 1).unsqueeze(0)
    out = torch.sum(hidden_states_new * weight_b, dim=1, keepdim=True)
    out = F.silu(out)
...
def causal_conv1d_update_npu(
    ...
    conv_state: (..., state_len, dim), where state_len >= width - 1
        NOTE: v2 keeps the persistent cache in channels-last layout
        (NSLOT, state_len, dim) to avoid per-step transpose.
    weight: (dim, width)
    bias: (dim,)
    ...
    pad_slot_id: int
            ...
            in this case, the kernel will not process entries at
            indices 0 and 3
    out: (batch, dim) or (batch, seq_len, dim)
    """
    if validate_data:
        assert cache_seqlens is None  # not implemented yet - ok for vLLM
        assert pad_slot_id is not None
        # channels-last: x is (batch, seq_len, dim); feat-dim is contiguous
        assert x.stride(-1) == 1
    ...
    squeeze_axis = None
    if unsqueeze:
        # make it (batch, 1, dim) with seqlen == 1
        x = x.unsqueeze(1)
        squeeze_axis = 1
    batch, seqlen, dim = x.shape
    _, width = weight.shape
    # v2 expects channels-last conv_state (..., state_len, dim).
    # If the caller (e.g. memory_pool not rolled back to origin) passes
    # channels-first (..., dim, state_len), transpose it here so the rest of
    # the function only deals with one layout.
    if conv_state.size(1) == dim:
        conv_state = conv_state.transpose(1, 2).contiguous()
    elif conv_state.size(2) != dim:
        raise ValueError(
            f"conv_state shape {conv_state.shape} is neither "
            f"channels-first (..., {dim}, state_len) nor "
            f"channels-last (..., state_len, {dim})"
        )
    # conv_state: (..., state_len, dim), where state_len >= width - 1
    num_cache_lines, state_len, _ = conv_state.size()
    ...
    if cache_seqlens is None and num_accepted_tokens is None:
        # v2: PyTorch path now consumes channels-last conv_state directly.
        ...
        # Triton kernel still expects channels-first (batch, dim, seqlen) inputs.
        # If caller passed channels-last, transpose before launching.
        if conv_state.size(1) != dim:
            conv_state = conv_state.transpose(1, 2).contiguous()
            (
                stride_istate_seq,
