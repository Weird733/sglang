...
class Sampler(nn.Module):
    def __init__(self):
        ...
        # Generate exponential noise on a side stream while the model is running.
    ...
    def prepare_async_exponential(
        ...
        """Enqueue Exp(1) noise before model forward on a dedicated NPU stream.
        ...
        if self._async_exponential_stream is None:
            ...
            logger.info("Enabled asynchronous NPU exponential sampling")
        ...
        with torch.npu.stream(self._async_exponential_stream):
            q = self._async_exponential_q
            if (
                q is None
                or tuple(q.shape) != expected_shape
                or q.dtype != torch.float32
                or q.device.type != torch.device(device).type
            ):
                q = torch.empty(
                    expected_shape,
                    dtype=torch.float32,
    ...
    def _sample_with_async_exponential(
        ...
        """Consume precomputed Exp(1) noise using the exponential-race identity."""
        ...
        if (
            q is None
            or q.shape != probs.shape
            or q.dtype != probs.dtype
            or q.device != probs.device
        ):
            logger.warning(
                "Async exponential buffer does not match probs; falling back to "
                "torch.multinomial (q=%s/%s/%s, probs=%s/%s/%s)",
                None if q is None else tuple(q.shape),
                None if q is None else q.dtype,
                None if q is None else q.device,
                tuple(probs.shape),
                probs.dtype,
                probs.device,
            )
        ...
        sampled_index = torch.div(probs, q).argmax(dim=-1)
