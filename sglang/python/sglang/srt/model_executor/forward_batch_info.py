...
class ForwardBatch(ForwardBatchDeepSeekMHAMixin):
    ...
    def _compute_mrope_positions(
        ...
        mm_inputs = batch.multimodal_inputs
        if get_global_server_args().rl_on_policy_target is not None or all(
                mm_input is None for mm_input in mm_inputs
        ):
            # Text-only batch, no speculative decoding here (see
            # _compute_spec_mrope_positions for that path). `self.positions`
            # was already computed a few lines above in init_new, directly
            # on model_runner.device via clamp_position() (decode) or
            # compute_position() (extend/mixed) -- both native on-device ops
            # or a fused triton kernel, no CPU round trip. mRoPE degenerates
            # to plain RoPE for text (t == h == w), so reuse it instead of
            # rebuilding + H2D-copying a (3, N) tensor from a per-request
            # CPU loop.
