...
from sglang.srt.utils import is_cuda, next_power_of_2, print_warning_once
...
class EagleVerifyInput(SpecInput, EagleVerifyInputV2Mixin):
    ...
    def verify(
        ...
        # Sample tokens. Force greedy sampling when the tree kernel is unavailable.
        is_all_greedy = sampling_info.is_all_greedy
        if (not is_all_greedy) and (not TREE_SPEC_KERNEL_AVAILABLE):
            print_warning_once(
                "Tree speculative sampling kernel unavailable on this build/device "
                "(including NPU). Falling back to greedy verification. "
